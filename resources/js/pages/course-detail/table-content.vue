  ...
  <div>
    <section id="CourseList" class="flex flex-col w-full rounded-[30px] p-[30px] gap-[30px] bg-[#F8FAFB]">
      <div class="header flex items-center justify-between">
        ...
        <RouterLink :to="`/course/create-contents/${props.courseId}`"
          class="w-fit rounded-full p-[14px_20px] font-semibold text-[#FFFFFF] bg-[#662FFF] text-nowrap">
          Tambah Materi
        </RouterLink>
      </div>
      <ContentItem v-for="(content, index) in currentContents.data" :key="content?.id || index" :id="content?.id"
        :index="((currentContents.current_page - 1) * currentContents.per_page) + index + 1" :type="content?.type"
        :title="content?.title" :courseId="content?.course_id" @delete-content="handleDeleteContent" />

      <div id="Pagination" class="flex items-center gap-3" v-if='currentContents.last_page > 1'>
        <!-- Previous button -->
        <button type="button" @click='goToPage(pagination - 1)' :disabled="pagination === 1"
        ...
        <!-- Page numbers -->
        <button type="button" v-for='page in currentContents.last_page' :key='page' @click='goToPage(page)'
        ...
        <!-- Next button -->
        <button type="button" @click='goToPage(pagination + 1)' :disabled="pagination === currentContents.last_page"
          class="flex shrink-0 w-9 h-9 rounded-full items-center justify-center text-center transition-all duration-300 hover:bg-[#662FFF] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed bg-[#EAEAEA] text-[#662FFF]">
          <span class="font-semibold text-sm leading-[21px]">›</span>
        </button>
      </div>
...
const handleDeleteContent = async (contentId) => {
  ...
  Swal.fire({
    ...
    text: "Materi ini akan dihapus dari kelas!",
    icon: "warning",
      ...
      try {
        ...
        if (response) {
          Swal.fire({
            ...
            text: "Materi kursus telah dihapus.",
            icon: "success",
            confirmButtonColor: "#662FFF",
            timer: 1500
          });
